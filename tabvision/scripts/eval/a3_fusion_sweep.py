...
def _raw_events_cached(
    media_path: Path, session: SessionConfig, backend_name: str, cache_dir: Path
) -> list[AudioEvent]:
    ...
    backend = make_audio_backend(backend_name)
    demuxed = demux(str(media_path))
    events = list(backend.transcribe(demuxed.wav, demuxed.sample_rate, session))
...
def main(argv: list[str] | None = None) -> int:
    ...
    for clip in payload.get("clips") or []:
        ...
        raw = _raw_events_cached(media, _session_from_clip(clip), args.backend, args.cache_dir)
