...
**Last updated:** 2026-08-02 (TabCNN complementarity evaluation completed)
...
### TabCNN complementarity evaluation addendum (frozen 2026-07-29; completed 2026-08-02)

This review covers only the evaluation described in
`docs/plans/2026-07-29-tabcnn-complementarity-experiment.md`. Nothing below is
registered, shipped, redistributed, or added to the default dependency path.

| Artifact | Frozen identity | License evidence | Evaluation decision |
|---|---|---|---|
| SynthTab TabCNN x4 `SynthTab-Pretrained.pt` | source commit `6136f79d04d8627f1fec57d31cd5667db9854bbc`; 52,573,995 bytes; SHA-256 `a5a0812844edd1dd9540170d2bcadb543b83de2066bd18b18ac13d666d511318` | Repository code is CC-BY; the SynthTab dataset used to train the published checkpoint is described as CC-BY-NC-4.0. The weight grant is therefore ambiguous beyond personal non-commercial use. | ⚠️ **internal evaluation completed; `do_not_integrate`**. No redistribution or automatic integration. Commercial or redistributed use requires separate clearance. |
| DAFx-24 GuitarProFX TabCNN, executed as `tabcnn-gpfx.onnx` | official source commit `f50309ad06dc734ddae5e3a0eda756fca221e2e7`; official checkpoint MD5 `ce168b2cd426f81a2a78499214e40605`; ONNX source commit `c15524a6944febe68129d26c2a89eca455b5499d`; 3,339,568 bytes; SHA-256 `8d9ce59157bdab37fb4816d32d7f29f3da0cdbf3c7876707c819af4d1f88e6b7` | Zenodo record 11406378 is CC-BY-4.0; official source is CC0. Attribution and derivative/export provenance are required. | ✅ evaluation completed, ⚠️ **`do_not_integrate` and no redistribution**. GuitarSet is development-overlapped. Official-checkpoint-to-ONNX numerical equivalence was not locally verified, so the provenance gate failed. |
| `tabcnn-cqt.bin` reference artifact | 696,312 bytes; SHA-256 `4e5dfa1f10f76545a30cbfd3224431503dbad943b1def78624632284e6df597a`; mirror commit `c15524a6944febe68129d26c2a89eca455b5499d` | CC-BY-4.0 mirror posture; attribution required. | ✅ hash-pinned reference/validation artifact only. It is **not loaded**; the executed front end is librosa CQT. |

The executed runtime was isolated to the repository virtual environment plus
an external evaluation probe environment; no TabCNN runtime was added to
`pyproject.toml`. Both families completed the frozen evaluation on 2026-08-02
and were rejected for integration. The unavailable official DAFx checkpoint
and failed official-equivalence check block `evidence_positive`, not honest
descriptive use. See
...
| Guitar-TECHS | Phase 0 (eval) / 1.5 / 7 | CC-BY-4.0 (Zenodo record 14963133) | ✅ eval-only | arXiv:2501.03720 — 3 electric guitarists, 5h12m multi-mic + DI; per-string 6-track MIDI. The TabCNN experiment uses the fixed 82-WAV/JAMS `guitar-techs` slice from `ryangowe/guitar-chord-mix` at commit `4448053ced18e67a9f66bfab47ac2de3cc0b4521`; two Hugging Face metadata files authenticate that revision and the scored manifest hashes every input. Media is not redistributed; required attribution must appear in the public README. |
| EGSet12 | TabCNN complementarity eval | CC-BY-4.0 (Zenodo record 11406378) | ✅ eval-only, scored 2026-08-02 | Twelve official WAV/JAMS pairs used by the DAFx-24 work. The corpus was acquired from Zenodo after the earlier connectivity failure; the frozen manifest pins every byte count and MD5, and all 12 pairs were scored. No media is redistributed. |
...
| SynthTab | **re-opened 2026-07-20** as candidate training substrate (not locally trained) | dataset CC-BY-NC-4.0 (code CC-BY-4.0) | ⚠️ **NC** (personal posture) | github.com/yongyizang/SynthTab. Was DROPPED 2026-05-13 because the NC clause taints derived weights under the old portfolio rule. **2026-07-20 posture amendment:** NC-derived weights are acceptable when labeled NC. No raw SynthTab dataset was acquired for local training; the upstream pretrained checkpoint was acquired only for the internal evaluation recorded in the model addendum above. Any future locally trained weight must be recorded here as **NC**. |
...
| Roboflow `s-workspace-y3mjn/guitar-fret-6pt` | Phase E2 (fret-keypoint training) | **CC BY 4.0** | ✅ | **Verified 2026-07-27**, two ways: the live Roboflow Universe page, and `roboflow: {license: CC BY 4.0}` written into the downloaded `data.yaml` itself. Keypoint detection, `kpt_shape: [6, 3]`, classes `fret` / `nut`, 926 images (710/144/72). The six keypoints per instance are the wire's intersections with the six strings, so the export yields the string **and** fret axes directly. Downloaded 2026-07-27 via the Roboflow SDK to `~/.tabvision/data/datasets/roboflow-s-workspace-y3mjn-guitar-fret-6pt-v1`; media never committed. **Attribution owed to two parties:** the image set is almost certainly a re-annotation of `b101/guitar-3` (identical 926 count and 710/144/72 split), which CC BY 4.0 permits — so credit both "guitar 3" by b101 and "guitar-fret-6pt" by s-workspace-y3mjn, each CC BY 4.0, in the public README. |
...
| S1b symbolic corpus (`synthtab_all.npz`) | Q2 contextual-assigner pretraining substrate | **CC-BY-NC-4.0 (inherited)** | ✅ derived 2026-07-22, local only | Per-track note sequences (34,621 tracks / 34.06M notes) extracted from the SynthTab JAMS above by `scripts/eval/s1b_extract_symbolic.py`; same parse as the S1a count priors, order preserved. Lives under `$TABVISION_DATA_ROOT/models/s1b_symbolic/`, git-ignored, never committed. Any model trained on it inherits NC and must be labeled here before registration. |
| S1b contextual string model (`context_v2.pt`) | Q2 probe checkpoint — contextual string assigner | **CC-BY-NC-4.0 (inherited)** | ✅ derived 2026-07-22, local only, probe artifact | 413,958-param transformer trained on the S1b symbolic corpus above (`s1b_train_context.py`). Gate FAILED (+0.0302 vs +0.05 bar), so it is **not registered** and never enters `auto`. Lives under `$TABVISION_DATA_ROOT/models/s1b_symbolic/`, git-ignored, never committed. Registration would require an NC label here plus the standard OOF → player-05 discipline. |
| S1b fine-tuned folds (`context_v2_oof_00..04.pt`) | Q2 stage-2 probe checkpoints (leave-one-player-out) | **CC-BY-NC-4.0 (inherited)** | ✅ derived 2026-07-22, local only, probe artifacts | Fine-tuned from `context_v2.pt` on GuitarSet (CC-BY-4.0) dev players 00-04, one fold per held-out player; NC is inherited from the SynthTab-pretrained initialization, not from GuitarSet. Gate FAILED (+0.0467 vs +0.05), Q2 closed — **not registered**, never in `auto`. Git-ignored under `$TABVISION_DATA_ROOT/models/s1b_symbolic/`. |
...
| scipy | BSD-3-Clause | ✅ verified 2026-07-27 | In `dev` and the `audio-baseline` extra. `scipy.stats.rankdata` backs the tie-corrected AUC in `scripts/eval/n3_physics_review_probe.py`; permissive, no obligation beyond retaining the BSD notice |
| PyTorch 2.11.0 | BSD-3-Clause | ✅ eval-only | Executes the checksum-pinned SynthTab checkpoint through a sanitized local module. Already present in the dev/audio environment; not added by this experiment. |
| librosa 0.11.0 | ISC | ✅ eval-only | Executes the frozen 22,050 Hz CQT geometry for both TabCNN families; exact version enters the posterior cache identity. |
| ONNX Runtime 1.23.2 | MIT | ✅ eval-only | Loaded only from `$TABVISION_DATA_ROOT`'s external TabCNN probe environment for the DAFx ONNX transport; absent from `pyproject.toml` and every default route. |
