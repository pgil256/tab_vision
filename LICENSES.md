...
| PDMX (`no_license_conflict` subset, 222,856 songs) | v1.1 A15 (sequence-prior corpus candidate) | **CC-BY (dataset) over PD-Mark/CC0 scores** | ✅ cleared 2026-07-02, yield pending | arXiv 2409.10831 / Zenodo 10.5281/zenodo.15571083 / github.com/pnlong/PDMX. **License review 2026-07-02 (read-only, no downloads): CLEAR-WITH-CONDITIONS** for a shipped derived n-gram prior — same artifact class as the GuitarSet-derived `guitarset-v1`/`guitarset-seq-v1` priors. Conditions: attribute in README + here; `no_license_conflict` subset only; never redistribute score content (count tables only). ⚠️ Target the **MXL files** (MuseScore 3.6.2 native exports — preserve `<technical><string>/<fret>`), NOT the MusicRender JSON (verifiably drops tab data). Guitar-TAB yield unquantified (est. hundreds–few-thousand scores, classical-leaning); resolve via metadata-CSV-first plan in `docs/2026-07-02-pdmx-license-yield-review.md` (Zenodo unreachable 2026-07-02, retry). |
