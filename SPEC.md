...
- **Personal non-commercial licensing (amended 2026-07-20; supersedes "portfolio-friendly licensing").** The user has designated TabVision a **personal, non-commercial application** and explicitly authorized accepting non-commercial licenses in the shipping default (chat directive 2026-07-20; see DECISIONS.md same date). Every dependency, model weight, and dataset in the **shipping default** pipeline must permit *personal non-commercial use with public source display* (public repo, README, demo). Permissive licenses remain preferred; **CC-BY-NC / CC-BY-NC-SA datasets and weights are now acceptable in the default pipeline and as training substrate for shipped derived artifacts**, provided (a) use stays non-commercial, (b) attribution and ShareAlike obligations are honored per artifact in LICENSES.md, and (c) each NC-derived artifact is labeled NC in LICENSES.md so any future commercialization knows exactly what must be replaced. **Private/user recordings remain banned from all training/eval/label roles** (2026-06-11 cleanup, explicitly retained by the user 2026-07-20) — **with one narrow carve-out (user directive 2026-08-02): opt-in local personal position priors.** The user's own recordings may, strictly opt-in per run (`--harvest-personal-labels`), be harvested into `(pitch, string, fret)` labels — audio pitch joined against FretCam's locked position windows, kept only when exactly one playable candidate is consistent — and aggregated into a **local** personal position-prior artifact (`scripts/train/build_personal_prior.py`, consumed via `--position-prior <path>.json`). **Widened same day (second user directive 2026-08-02): gold-tab sessions.** The user may pair their own recordings with self-supplied reference tabs (`scripts/train/ingest_gold_session.py`) **or bank a transcription they corrected in the local studio editor (third directive, same day: the "Bank gold" button posts to the local-only `/jobs/<id>/gold-session` endpoint, which exists only when `studio.ps1` sets `TABVISION_PERSONAL_ROOT`)**; the aligned ground-truth labels may feed (a) the same personal position-prior store and (b) a **local video-training corpus** (`<data-root>/personal/video_corpus/`) usable as training substrate for **local-only** video-analysis artifacts. Personal artifacts, label stores, and the video corpus are local to the user's machine: never shipped, never registered as defaults, never used in eval corpora or published figures, and never a substrate for any shipped or published artifact. Everything else about the ban stands. Rationale and pricing (+0.0305 in-sample ceiling, Track C): DECISIONS.md 2026-08-02. Phase 0 produced the initial license map; the license CI now verifies the default against the amended posture.
